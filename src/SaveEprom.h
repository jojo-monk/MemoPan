...
void setup() {
    Serial.begin(9600);
    EEPROM.begin(512); // Pour ESP32/ESP8266
    
    // Définir les valeurs par défaut
    uint8_t valeursDefaut[8] = {0, 1, 2, 3, 4, 5, 6, 7};
    
    // Vérifier si des données valides existent
    if (!eeprom.donneesValides()) {
        Serial.println("Aucune donnée valide trouvée, initialisation...");
        eeprom.initialiser(valeursDefaut);
    }
...
void exempleAccesDonnees() {
    // MÉTHODE 1 : Charger toutes les variables dans un tableau
    uint8_t mesVariables[8];
    if (eeprom.charger(mesVariables)) {
        Serial.println("Variables chargées avec succès :");
        for (int i = 0; i < 8; i++) {
            Serial.print("Variable ");
            Serial.print(i);
            Serial.print(": ");
            Serial.println(mesVariables[i]);
        }
        
        // Accès aux variables spécifiques par index
        uint8_t highScore = mesVariables[0];
        uint8_t son = mesVariables[1];
        uint8_t gamme = mesVariables[2];
        // etc...
    }
    
    // MÉTHODE 2 : Charger une variable spécifique
    uint8_t highScore;
    if (eeprom.chargerVariable(0, highScore)) {
        Serial.print("High Score: ");
        Serial.println(highScore);
    }
    
    // MÉTHODE 3 : Obtenir directement une variable (plus simple)
    uint8_t volume = eeprom.obtenirVariable(7);
    Serial.print("Volume: ");
    Serial.println(volume);
    
    // Modifier une variable
    eeprom.sauvegarderVariable(0, 100); // Nouveau high score
    
    // Vérifier le changement
    Serial.print("Nouveau High Score: ");
    Serial.println(eeprom.obtenirVariable(0));
}
...
// FONCTIONS D'ACCÈS PRATIQUES (à ajouter selon vos besoins)



// etc... pour toutes vos variables
*/
