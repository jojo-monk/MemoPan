...
const SoundMode soundModes[NUM_SOUND_MODES] = {
  { SYNTH, "Sine",    { .synth = { WAVEFORM_SINE,     0.6, 10, 50, 0.7, 1000, 0.6, 90, 0.75, 0.2 } } },
  { SYNTH, "Saw",     { .synth = { WAVEFORM_SAWTOOTH, 0.4, 100, 30, 0.5, 400, 2, 120, 0.51, 0.314 } } },
  { SYNTH, "Square",  { .synth = { WAVEFORM_SQUARE,   0.5, 20, 50, 0.1, 200, 1.5, 10, 0.5, 0.25 } } },
  { SYNTH, "HandPan", { .synth = { WAVEFORM_TRIANGLE, 0.6, 30, 1800, 0.3, 1000, 0.1, 5, 1.3, 0.1 } } },
  { STRING,"String",  { .string= { 0.8 } } },
  { DRUM,  "Drum",    { .drum  = { 50, 0.8, 0.55 } } },
  { SAMPLE, "Sample",  { .sample= { {"kick", "snare", "openHH", "closedHH", "cowbell", "lowConga", "rimShot", "tabla1", "tabla2", "tabla3", "tabla4", "tabla5" } } } }
};
...
EffectsParams effects = defaultEffects;
...
public:
  ...
  void playTouchSound(int touchIndex, uint16_t pression, uint8_t velocite, int octaveShift);
...
SoundManager::SoundManager() :
  ...
  FxMixerTodrywet(fxMixer, 0, dryWetMix, 1),
...
void SoundManager::playTouchSound(int touchIndex, uint16_t pression = 150, uint8_t velocite = 150, int octaveShift = 0) {
    if (touchIndex < 0 || touchIndex >= NUM_TOUCH_PADS) return;
    ...
    switch(sm.type) {
      case SYNTH:
        ...
        //polyEnv[voiceIndex].release(); // vélocité rapide = release court
        }
